---
layout: page
title: scientific service
display_title: Scientific Service
permalink: /service/
nav: true
nav_order: 2
_styles: >
  .post article h4 {
    font-size: 1.1rem;
  }
---

[Organization](#organization) &middot; [Program Committees](#program-committees-conferences) &middot; [Editor for Journals](#editor-for-journals) &middot; [Reviews](#reviews) &middot; [Tutorials](#tutorials)

## Organization

#### Current

- Organizer of the Dagstuhl Seminar on Blending Knowledge Graphs and Process Mining for Procedural Knowledge Management, 2027
- Steering Committee member of EMISA, since 2025

#### Past

- Organizer of the Working Conference on Business Process Modeling, Development and Support (BPMDS), 2023 — 2026
- Workshop Chair at the International Conference on Business Process Management (BPM), 2025
- Mini-track chair at the Hawaii International Conference on System Sciences (HICSS), 2022 — 2025
- Doctoral Consortium Chair at the International Conference on Process Mining (ICPM), 2024
- Doctoral Consortium Chair at the International Conference on Business Process Management (BPM), 2024
- Organizer of the Workshop on Natural Language Processing for Business Process Management (NLP4BPM), 2022 — 2024
- Organizer of the 1st Process Analytics Research Seminar (PARS), 2022

## Program Committees (Conferences)

#### Current

- Int. Conf. on Business Process Management (BPM), Senior PC member, since 2025
- Int. Conf. on Advanced Information Systems Engineering (CAiSE), since 2023
- Int. Conf. on Process Mining (ICPM), since 2021

#### Past

- Int. Conf. on Business Process Management (BPM), PC member, 2020 — 2024
- Int. Conf. on Cooperative Information Systems (CoopIS), 2022 — 2023
- Int. Conf. on Distributed and Event-Based Systems (DEBS), 2021 — 2022
- Working Conf. on Business Process Modeling, Development, and Support (BPMDS), 2019 — 2022
- Int. Joint Conf. on Neural Networks (IJCNN), 2020

## Editor for Journals

- Guest Editor, Process Science — Best Process Science Conference Papers 2026 (in progress)
- Guest Editor, Process Science — [Best Process Science Conference Papers 2025](https://link.springer.com/collections/fcbfjejcab)
- Guest Editor, Software and Systems Modeling — [Special Issue of selected papers of the BPMDS Working Conference 2024](https://link.springer.com/article/10.1007/s10270-025-01347-5)
- Guest Editor, Business & Information Systems Engineering — [Special Issue on Exploring the (Mis)Match Between Real-World Processes and Event Data](https://link.springer.com/article/10.1007/s12599-025-00939-1#article-info), 2025
- Guest Editor, Software and Systems Modeling — [Special Issue of selected papers of the BPMDS Working Conference 2023](https://link.springer.com/article/10.1007/s10270-024-01248-z)

## Reviews

#### Journals

- ACM Transactions on the Web
- Advanced Engineering Informatics
- Artificial Intelligence in Medicine
- Business & Information Systems Engineering
- Computer Science Review
- Computer Standards & Interfaces
- Computers in Industry
- Computing
- Data Mining and Knowledge Discovery
- Decision Support Systems
- Expert Systems with Applications
- IEEE Transactions on Knowledge and Data Engineering
- IEEE Transactions on Services Computing
- Information Systems
- Journal of Biomedical Informatics
- Journal of Software: Evolution and Process
- Journal on Data Semantics
- Process Science
- Requirements Engineering
- SN Business & Economics
- Software and Systems Modeling

#### Conferences

- International Conference on Very Large Data Bases (VLDB), 2023
- European Conference on Information Systems (ECIS), 2018 — 2023
- Wirtschaftsinformatik (WI), 2017, 2022
- Hawaii International Conference on System Sciences (HICSS), 2021
- Business Process Modeling, Development, and Support (BPMDS), 2015, 2017
- Americas Conference on Information Systems (AMCIS), 2017
- ACM Symposium on Applied Computing (SAC), 2017
- International Conference on Business Process Management (BPM), 2016
- International Conference on Service Oriented Computing (ICSOC), 2015 — 2016

#### Funding Agencies

- German Research Foundation (DFG)
- Swiss National Science Foundation (SNSF)

## Tutorials

- IEEE Int. Conf. on Data Engineering (Utrecht, The Netherlands): _Privacy-Aware Analysis based on Data Series_, June 2024
- Int. Conf. on Business Process Management (Utrecht, The Netherlands): _Natural Language Processing for Business Process Analysis_, September 2023
- Int. Conf. on Business Process Management (Rome, Italy): _RuM: Declarative Process Mining, Distilled_, September 2021
- IEEE Big Data (Los Angeles, USA): _Process Mining: Leveraging Event Data to Understand and Improve Organizations_, December 2019
